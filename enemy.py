...
class Enemy:
    def __init__(self, landschaft: SpielLandschaft, batch: pyglet.graphics.Batch, animation):
    ...
    def _extrahiere_weg(self, landschaft: SpielLandschaft):
        ...
        # Startfeld in Spalte 0 finden
        start_felder = [f for f in weg_dict.values() if f.index_x == 0]
        start = min(start_felder, key=lambda f: f.index_y)
    ...
    def _aktualisiere_drehung(self, dx: float, dy: float):
        ...
        if abs(dx) > abs(dy):
            # Ritter läuft nach rechts
            if dx > 0:
                self.sprite.rotation = 0
            # Ritter läuft nach links
            else:
                self.sprite.rotation = 180
        else:
            # Ritter läuft nach oben
            if dy > 0:
                self.sprite.rotation = 270
            # Ritter läuft nach unten
            else:
